...
import { arrow_right, check_circle } from '@manifoldco/icons';
...
const NewResource: React.FunctionComponent<RouteComponentProps> = ({ location: { search } }) => {
  ...
  useEffect(() => {
    ...
    document.addEventListener('manifold-planUpdated', planChanged as EventListener);
    return () =>
      document.removeEventListener('manifold-planUpdated', planChanged as EventListener);
  }, [planID, features]);
  ...
  return (
    <Wrapper>
      <Deets>
        <manifold-product product-label={product} />
      </Deets>
      <PlanSelector>
      ...
      <Confirmation>
        ...
        <Button onClick={() => setStatus(LOADING)}>
          Purchase&nbsp;<manifold-data-product-name product-label={product} />
